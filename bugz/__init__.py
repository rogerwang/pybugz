...
class Bugz:
    ...
    def __init__(self, base, user = None, password = None, forget = False,
        ...
        self.config = load_config_by_url(base)
    ...
    def auth(self):
        ...
        req_url = urljoin(self.base, self.config['urls']['auth'])
        req_url += '?GoAheadAndLogIn=1'
        self.debug('Requesting %s' % req_url)
        req = Request(req_url, None, self.config['headers'])
        ...
        # perform login
        qparams = self.config['params']['auth'].copy()
        qparams['Bugzilla_login'] = self.user
        qparams['Bugzilla_password'] = self.password

        req_url = urljoin(self.base, self.config['urls']['auth'])
        req = Request(req_url, urlencode(qparams), self.config['headers'])
        if self.httpuser and self.httppassword:
            base64string = base64.encodestring('%s:%s' % (self.httpuser, self.httppassword))[:-1]
            req.add_header("Authorization", "Basic %s" % base64string)
        resp = self.opener.open(req)
    ...
    def search(self, query, comments = False, order = 'number',
        ...
        qparams = self.config['params']['list'].copy()
        if comments:
            qparams['long_desc'] = query
        else:
            qparams['short_desc'] = query

        qparams['order'] = self.config['choices']['order'].get(order, 'Bug Number')
        ...
        if status == None:
            ...
            qparams['bug_status'] = self.config['choices']['status']
        else:
        ...
        req_url = urljoin(self.base, self.config['urls']['list'])
        req_url += '?' + req_params
        self.debug('Requesting %s' % req_url)
        req = Request(req_url, None, self.config['headers'])
        if self.httpuser and self.httppassword:
        ...
        for field in rows[0]:
            if self.config['choices']['column_alias'].has_key(field):
                columns.append(self.config['choices']['column_alias'][field])
            else:
    ...
    def get(self, bugid):
        ...
        @rtype: ElementTree
        """
        qparams = self.config['params']['show'].copy()
        qparams['id'] = bugid

        req_params = urlencode(qparams, True)
        req_url = urljoin(self.base,  self.config['urls']['show'])
        req_url += '?' + req_params
        self.debug('Requesting %s' % req_url)
        req = Request(req_url, None, self.config['headers'])
        if self.httpuser and self.httppassword:
    ...
    def modify(self, bugid, title = None, comment = None, url = None,
        ...
        import time
        modified = []
        qparams = self.config['params']['modify'].copy()
        qparams['id'] = bugid
        qparams['knob'] = 'none'
        ...
        req_params = urlencode(qparams, True)
        req_url = urljoin(self.base, self.config['urls']['modify'])
        self.debug('Requesting %s' % req_url)
        req = Request(req_url, req_params, self.config['headers'])
        if self.httpuser and self.httppassword:
            base64string = base64.encodestring('%s:%s' % (self.httpuser, self.httppassword))[:-1]
            req.add_header("Authorization", "Basic %s" % base64string)
    ...
    def attachment(self, attachid):
        ...
        @rtype: dict
        """
        qparams = self.config['params']['attach'].copy()
        qparams['id'] = attachid

        req_params = urlencode(qparams, True)
        req_url = urljoin(self.base, self.config['urls']['attach'])
        req_url += '?' + req_params
        self.debug('Requesting %s' % req_url)
        req = Request(req_url, None, self.config['headers'])
        if self.httpuser and self.httppassword:
    ...
    def attach(self, bugid, title, description, filename,
        ...
        qparams = self.config['params']['attach_post'].copy()
        qparams['bugid'] = bugid
        ...
        req_url = urljoin(self.base, self.config['urls']['attach_post'])
        self.debug('Requesting %s' % req_url)
